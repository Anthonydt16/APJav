...
public class FicheFrais {
	...
	public FicheFrais(int unIdFiche , int unMois, int uneAnnee,  Date uneDateCreation, Date uneDateCloture, String unEtat, float unMontantDeclare  ,Utilisateur unUtilisateur) {
		this.idFiche = unIdFiche;
		this.mois = unMois;
		this.annee = uneAnnee;
		this.dateCreation = uneDateCreation;
		this.dateCloture = uneDateCloture;
		this.etat = unEtat;
		this.montantDeclare = unMontantDeclare;
	...
	{
		return switch (etat) {
			case "EC" -> "EC-En cours de saisie";
			case "CL" -> "CL-Clôturée";
			case "VA" -> "VA-Validée";
			case "MP" -> "MP-Mise en paiement";
			default -> "RB Remboursée";
		};
