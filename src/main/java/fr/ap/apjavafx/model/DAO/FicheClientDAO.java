    ...
    public static ArrayList <FicheClient> getAllLoueur() {
        ...
        try (PreparedStatement statement = DBConnex.getConnexion().prepareStatement("SELECT e.NOMENT as `nomEntreprise`, e.ADRESSEENT as `adresseEntreprise`, CONCAT(V.NOMVILLE,'/', P.NOMPAYS) as `vilePays`, e.TELENT as `telEntreprise`, e.EMAIL as `emailEntreprise`, CONCAT(l.NOM, ' ', l.PRENOM) AS `nomPrenomContact`, l.MAILCONTACT as `mailContact`, l.TELCONTACT as `telContact`" +
            ...
            try (ResultSet result = statement.executeQuery()) {
                if (result.next()) {
                    uneFicheClient = new FicheClient(result.getString("nomEntreprise"),result.getString("adresseEntreprise"),result.getString("vilePays"),result.getString("telEntreprise"),result.getString("emailEntreprise"),result.getString("nomPrenomContact"),result.getString("mailContact"),result.getString("telContact"));
