...
public class ControllerComptableListeFiches {
	...
	private void buttonOuvrirFicheComptableClick(ActionEvent e) {
		...
		if (index >= 0) {
			...
			try {
				...
				loader.setLocation(getClass().getResource("../view/view-comptable-fiche.fxml"));

				Parent comptableFicheLayout = loader.load();
	...
	private void remplissagetableViewListeFichesComptable() {
		...
		colIdFiche.setCellValueFactory(new PropertyValueFactory<>("idFiche"));
		colMoisFiche.setCellValueFactory(new PropertyValueFactory<>("moisAnnee"));
		colNomVisiteur.setCellValueFactory(new PropertyValueFactory<>("nomPrenomUtilisateur"));
		colEtatFiche.setCellValueFactory(new PropertyValueFactory<>("etatLong"));
